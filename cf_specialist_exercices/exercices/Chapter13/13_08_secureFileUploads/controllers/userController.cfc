component 
{
	...
	remote void function updateProfile( firstName, lastName, Email, Password, headshot )
	{
		try
		{
			var uploadedFile = fileUpload( getTempDirectory(), 
										   "headshot", 
										   "image/png,image/jpeg,image/pjpeg", 
										   "MakeUnique",
										   true );

			// check the extension of the uploaded file - mime types can be faked
			if( !listFindNoCase( "png,jpg,jpeg", uploadedFile.serverFileExt ) )
			{
				throw( "The uploaded file is not of type JPG or PNG." );
			}
			else 
			{
				// file appears to be legit, move it to the images folder
